...
<tr data-user-id="<?= (int) $m['id'] ?>">
    ...
    <td data-sort-value="<?= $isTeacher ? 'Enseignant' : 'Etudiant' ?>">
        <span class="badge <?= $isTeacher ? 'badge-teacher' : 'badge-student' ?>">
            <?= $isTeacher ? 'Enseignant' : 'Etudiant' ?>
        </span>
    </td>
    <td data-status-cell data-sort-value="<?= $m['is_active'] ? 'Actif' : 'Desactive' ?>">
        <?php if ($m['is_active']): ?>
            <span class="badge badge-active"><?= icon('check', '', 12) ?> Actif</span>
        <?php else: ?>
            <span class="badge badge-draft"><?= icon('lock', '', 12) ?> Desactive</span>
        <?php endif; ?>
    </td>
    <td data-action-cell>
        ...
        <form method="POST" action="/department-admin/users/set-active" data-ajax-action="set-active">
            ...
            <?php if ($m['is_active']): ?>
                <button class="btn danger sm" type="submit"><?= icon('lock', '', 13) ?> Desactiver</button>
            <?php else: ?>
                <button class="btn success sm" type="submit"><?= icon('check', '', 13) ?> Reactiver</button>
            <?php endif; ?>
        </form>
        <?php endif; ?>
    </td>
