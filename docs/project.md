...
- `users` — base account. Holds `department_id` (nullable FK to `departments`, `ON DELETE SET NULL`): a user belongs to **at most one** department, joined via a **department code** entered at registration. Researchers stay NULL (a researcher is a user but is not attached to a department). Super admins are not in this table at all (see below).
- `students`, `teachers`, `researchers`, `department_administrators` — vertical inheritance for roles (PK = `users.id`, `ON DELETE CASCADE`). Exclusivity enforced by `enforce_role_exclusivity()` (see [`02_triggers.sql`](../database/schema/02_triggers.sql)): a user holds **at most one** role — every role is mutually exclusive. A teacher with `is_specialised = TRUE` is an **habilitated** teacher (set by a department admin) and may import custom models into resources.
